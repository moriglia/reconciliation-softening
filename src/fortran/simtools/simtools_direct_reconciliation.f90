...
submodule (simtools) simtools_direct_reconciliation
  use ldpc, only : decoder_t, decoder_from_edge_table, decode, word_to_syndrome
...
contains
  module subroutine sim_direct(&
    ...
    integer :: i_snr, i_sim, i_bit, i_x, i, N_buff
    ...
    if (this_image() == 1) then
       ! print *, decoder%vnum, decoder%cnum, decoder%edge_num
       ! do i = 1, decoder%vnum
       !    print *, decoder%v_to_e(i)%edge_list
       ! end do
    end if
    sync all
    ...
    allocate(y(0:N_symb-1))
    allocate(llr(0:N-1))
    allocate(final_llr(0:N-1))
    ...
    snrloop : do i_snr = 1, nsnr
       ...
       simloop : do i_sim = 1, nloops
          ...
          do i_x = 0, N_symb-1
             do i_bit = 0, bps-1
                if (iand((x_i(i_x)*(x_i(i_x)+1)), b'11') == 0) then
          ...
          call decode(decoder, llr, synd, final_llr, d_iter)
          ...
          ! EARLY EXIT
          if ((count_ferr(i_snr)[1] > min_ferr) .and. ( count_words(i_snr)[1] > nloops/20)) then
             exit simloop
          end if
       end do simloop
       print *, "[", this_image(), "]: SNR", snr_dB_range(i_snr), &
            "LOOPS", i_sim, &
            "FRAME_ERRORS", count_ferr(i_snr)[1], &
            "BIT_ERRORS", count_berr(i_snr)[1]
    end do snrloop
    ...
    if (this_image() == 1) then
       do i = 1, nsnr
          ber(i) = real(count_berr(i), c_double)/real(count_words(i)*K, c_double)
          fer(i) = real(count_ferr(i), c_double)/real(count_words(i), c_double)
       end do
    end if
